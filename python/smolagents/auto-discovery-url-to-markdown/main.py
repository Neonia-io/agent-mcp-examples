...
class DiscoveryTool(Tool):
    ...
    def forward(self, query: str) -> str:
        ...
        async def call_mcp():
            try:
                result = await self.session.call_tool("neonia.sys.tool.discovery", arguments={"query": query})
...
class ExecuteTool(Tool):
    ...
    def forward(self, target_tool: str, payload: str) -> str:
        ...
        async def call_mcp():
            try:
                result = await self.session.call_tool("neonia.sys.tool.execute", arguments={"target_tool": target_tool, "payload": payload_dict})
                if result.isError: return f"Error: {result.content}"
                return "\n".join([c.text for c in result.content if c.type == "text"])
            except Exception as e:
                return f"Error: {e}"
