# Stateful Cloud Memory Example (Python / SmolAgents)
...
2. **Session 2 (New Day)**: A completely fresh SmolAgents instance is created with **zero** local conversation history. The user simply asks for the weather on Hoth. The agent autonomously uses the `recall` action, retrieves the persona rules, and responds accordingly.
