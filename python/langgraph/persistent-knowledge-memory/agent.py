...
async def main():
    ...
    url = "https://mcp.neonia.io/mcp?tools=neonia_sys_memory_lesson,neonia_sys_memory_search"
    ...
    async with streamable_http_client(url, http_client=client) as streams:
        async with ClientSession(streams[0], streams[1]) as session:
            ...
            @tool
            async def neonia_sys_memory_lesson(observation: str, root_cause: str, decision_rule: str, tags: list[str]) -> str:
                ...
                print(f"[Memory Tool] Agent executed: neonia_sys_memory_lesson")
                try:
                    result = await session.call_tool("neonia_sys_memory_lesson", arguments=arguments)
            ...
            @tool
            async def neonia_sys_memory_search(query: str) -> str:
                ...
                print(f"[Memory Tool] Agent executed: neonia_sys_memory_search (query='{query}')")
                try:
                    result = await session.call_tool("neonia_sys_memory_search", arguments=arguments)
                    if result.isError: return f"Error: {result.content}"
                    tool_output = "\n".join([c.text for c in result.content if c.type == "text"])
                    return tool_output
                except Exception as e: return f"Error: {e}"

            langchain_tools = [neonia_sys_memory_lesson, neonia_sys_memory_search]
            ...
            system_prompt = SystemMessage(content=(
                ...
                "CRITICAL: Before you answer ANY user prompt or take any actions, you MUST use `neonia_sys_memory_search` to fetch your architectural rules and lessons. "
                "Do not answer the user without searching your memory first! "
                "When explicitly asked to remember an architectural lesson, use `neonia_sys_memory_lesson`."
            ))
