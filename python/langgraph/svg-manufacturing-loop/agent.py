...
WORKFLOW RULES:
1. GENERATE: Call `neonia_util_svg_generator` with the user's prompt. 
...
2. VALIDATE: You MUST instantly call `neonia_util_svg_validator` in your NEXT action. Pass the URI string EXACTLY as received into the `svg_resource_uri` parameter. 
...
3. EVALUATE & HEAL (The Loop): 
   ...
   - If `is_valid` is false: Call `neonia_util_svg_generator` AGAIN. 
...
async def main():
    ...
    url = "https://mcp.neonia.io/mcp?tools=neonia_util_svg_generator,neonia_util_svg_validator"
    ...
    async with streamable_http_client(url, http_client=client) as streams:
        async with ClientSession(streams[0], streams[1]) as session:
            ...
            @tool
            async def neonia_util_svg_generator(prompt: str, style_mode: str, stroke_width: float = 1.0, validation_feedback: dict = None) -> str:
                ...
                print(f"\n[Autonomy] Agent calls neonia_util_svg_generator with args:\n{json.dumps(arguments, indent=2)}")
                try:
                    result = await session.call_tool("neonia_util_svg_generator", arguments=arguments)
                    if result.isError: return f"Error: {result.content}"
                    tool_output = "\n".join([c.text for c in result.content if c.type == "text"])
                    print(f"[Result] from neonia_util_svg_generator: {tool_output}")
            ...
            @tool
            async def neonia_util_svg_validator(svg_resource_uri: str = None, svg_content: str = None, check_closed_paths: bool = True, check_intersections: bool = True, check_continuity: bool = True, min_segment_length: float = 0.1) -> str:
                "A strict geometric linter for SVG code. Mathematically detects unclosed paths, self-intersections."
                
                print(f"\n[Autonomy] Agent calls neonia_util_svg_validator with URI: {svg_resource_uri or svg_content}")
                ...
                try:
                    result = await session.call_tool("neonia_util_svg_validator", arguments=arguments)
                    if result.isError: return f"Error: {result.content}"
                    tool_output = "\n".join([c.text for c in result.content if c.type == "text"])
                    print(f"[Result] from neonia_util_svg_validator: {tool_output}")
                    return tool_output
                except Exception as e: return f"Error: {e}"

            langchain_tools = [neonia_util_svg_generator, neonia_util_svg_validator]
