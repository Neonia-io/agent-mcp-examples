# Chained JSON JQ Filter Example (LangGraph)
...
1. The agent connects to `mcp.neonia.io/mcp?tools=neonia.web.json.fetch,neonia.data.jq.filter`.
2. It dynamically wraps the MCP tools into LangChain `@tool` instances.
