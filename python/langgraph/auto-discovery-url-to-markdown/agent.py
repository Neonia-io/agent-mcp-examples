...
async def main():
    ...
    # neonia_sys_tool_discovery and neonia_sys_tool_execute are system tools that are available by default.
    ...
    async with streamable_http_client(url, http_client=client) as streams:
        async with ClientSession(streams[0], streams[1]) as session:
            ...
            @tool
            async def neonia_sys_tool_discovery(query: str) -> str:
                ...
                try:
                    result = await session.call_tool("neonia_sys_tool_discovery", arguments={"query": query})
                    if result.isError: return f"Error: {result.content}"
                    return "\n".join([c.text for c in result.content if c.type == "text"])
                except Exception as e: return f"Error: {e}"

            @tool
            async def neonia_sys_tool_execute(target_tool: str, payload: dict) -> str:
                ...
                try:
                    result = await session.call_tool("neonia_sys_tool_execute", arguments={"target_tool": target_tool, "payload": payload})
                    if result.isError: return f"Error: {result.content}"
                    return "\n".join([c.text for c in result.content if c.type == "text"])
                except Exception as e: return f"Error: {e}"

            langchain_tools = [neonia_sys_tool_discovery, neonia_sys_tool_execute]
            ...
            system_prompt = SystemMessage(content=(
                ...
                "If you need a tool you do not have, use `neonia_sys_tool_discovery` to find it, "
                "and then explicitly run it using `neonia_sys_tool_execute` without asking the user. "
