...
async def main():
    ...
    # neonia.sys.tool.discovery and neonia.sys.tool.execute are system tools that are available by default.
    ...
    async with streamable_http_client(url, http_client=client) as streams:
        async with ClientSession(streams[0], streams[1]) as session:
            ...
            @tool
            async def neonia.sys.tool.discovery(query: str) -> str:
                ...
                try:
                    result = await session.call_tool("neonia.sys.tool.discovery", arguments={"query": query})
                    if result.isError: return f"Error: {result.content}"
                    return "\n".join([c.text for c in result.content if c.type == "text"])
                except Exception as e: return f"Error: {e}"

            @tool
            async def neonia.sys.tool.execute(target_tool: str, payload: dict) -> str:
                ...
                try:
                    result = await session.call_tool("neonia.sys.tool.execute", arguments={"target_tool": target_tool, "payload": payload})
                    if result.isError: return f"Error: {result.content}"
                    return "\n".join([c.text for c in result.content if c.type == "text"])
                except Exception as e: return f"Error: {e}"

            langchain_tools = [neonia.sys.tool.discovery, neonia.sys.tool.execute]
            ...
            system_prompt = SystemMessage(content=(
                ...
                "and then explicitly run it using `neonia.sys.tool.execute` without asking the user. "
