...
// Configuration
const ALLOWED_TOOLS = ["neonia_util_svg_generator", "neonia_util_svg_validator"];
...
async function main() {
    ...
    const transport = new StreamableHTTPClientTransport(new URL("https://mcp.neonia.io/mcp?tools=neonia_util_svg_generator,neonia_util_svg_validator"), {
        ...
        tools[mcpTool.name] = aiTool({
            ...
            execute: async (args: Record<string, unknown>) => {
                ...
                try {
                    ...
                    if (mcpTool.name === 'neonia_util_svg_generator') {
                        try {
                        ...
                        responseText += `\n\n[SYSTEM DIRECTIVE]: SVG generated successfully. You MUST now execute the 'neonia_util_svg_validator' tool using svg_resource_uri="${latestSvgUri}" to geometrically validate it. DO NOT answer the user or end the task until validation passes.`;
                    }
    ...
    while (!isValid && loopCount < 10) {
        ...
        const valResult = result.toolResults.find(tr => tr.toolName === 'neonia_util_svg_validator');
        if (valResult) {
            try {
        ...
        if (!isValid && result.toolCalls.length === 0) {
            ...
            messages.push({
                role: 'user',
                content: `You stopped without achieving a successful validation (is_valid: true). If you generated an SVG, you MUST now use the neonia_util_svg_validator tool. If it failed, you MUST generate a new SVG.`
            });
        }
    }
