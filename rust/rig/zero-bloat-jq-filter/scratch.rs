...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let url = "https://mcp.neonia.io/mcp?tools=neonia.sys.tool.discovery,neonia.sys.tool.execute";
    ...
    let response = agent.prompt("I need to fetch this URL as markdown: https://example.com. Please use discovery to find a tool, then execute it.").await?;
    println!("Response: {}", response);
    
    let tools2 = client.list_tools(Default::default()).await?.tools;
    println!("Final tools: {:?}", tools2.iter().map(|t| t.name.clone()).collect::<Vec<_>>());
